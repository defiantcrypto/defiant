...
{
    // Convert Defiant:// to Defiant:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Defiant://"))
    {
        uri.replace(0, 12, "Defiant:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Defiant.lnk";
}
...
{
    return GetAutostartDir() / "Defiant.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Defiant-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Defiant-qt [" + tr("command-line options") + "]                     " + "\n";
