...
int nAnonymizeDefiantAmount = 500;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Defiant
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Defiant
    // Mac: ~/Library/Application Support/Defiant
    // Unix: ~/.Defiant
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Defiant";
#else
    fs::path pathRet;
...
void createConf()       //Automatic defiant.conf generation
{
    srand(time(NULL));
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "defiant.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "defiantd.pid"));
